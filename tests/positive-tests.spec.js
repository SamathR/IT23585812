...
test.describe('Positive Functional Tests – Singlish to Sinhala', () => {
  ...
  test('POS_FUN_0002 - Simple present tense statement', async ({ page }) => {
    await page.goto('https://www.swifttranslator.com/', { timeout: 60000 });
    await page.fill('textarea', 'Mama Gedhara Yanawa');
    await page.waitForTimeout(3000);
  ...
  test('POS_FUN_0009 - Day-to-day expression', async ({ page }) => {
    ...
    await page.waitForTimeout(3000);
    await expect(page.locator('body')).toContainText('මට නිදිමතයි');
  });
  ...
  test('POS_FUN_0016 - Convert multi-line sentence', async ({ page }) => {
    ...
    await page.waitForTimeout(3000);
    await expect(page.locator('body')).toContainText('මම අද ගෙදර ඉන්නේ');
  });
  ...
  test('POS_FUN_0017 - Convert long paragraph with scheduling update', async ({ page }) => {
    ...
    await expect(page.locator('body')).toContainText('මම අද office යද්දි traffic එක ගොඩක් තිබුන නිසා');
  });
  ...
  test('POS_FUN_0021 - Convert plural pronoun activity sentence', async ({ page }) => {
    ...
    await page.waitForTimeout(3000);
    await expect(page.locator('body')).toContainText('එයාලා වාර්ශික චාරිකාව ගියා');
  });
  ...
  test('POS_FUN_0024 - Convert sentence with currency and number format', async ({ page }) => {
    ...
    await page.waitForTimeout(3000);
    await expect(page.locator('body')).toContainText('පොත් පහක්');
  });
...
test.describe('UI Tests – Singlish to Sinhala', () => {
  ...
  test('POS_UI_0001 - Verify real-time output updates as user types', async ({ page }) => {
    await page.goto('https://www.swifttranslator.com/', { timeout: 60000 });
    ...
    expect(bodyText).toContain('ම');
    
    await textarea.fill('ma');
    await page.waitForTimeout(1000);
  ...
  test('NEG_UI_0001 - Verify input field clears properly', async ({ page }) => {
    await page.goto('https://www.swifttranslator.com/', { timeout: 60000 });
    
    const textarea = page.locator('textarea');
    
    // Enter text
    await textarea.fill('mama gedara yanavaa');
